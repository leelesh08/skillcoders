...
esbuild.build({
  entryPoints: ['src/components/QuizModal.tsx'],
  bundle: true,
  outdir: 'tmp/esbuild-quiz',
  platform: 'browser',
  format: 'esm',
  sourcemap: false,
  ...
  console.log('esbuild: quiz build completed successfully');
}).catch((err) => {
  console.error('esbuild: quiz build failed');
  console.error(err);
  process.exit(1);
});
